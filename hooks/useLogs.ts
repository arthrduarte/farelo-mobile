...
export function useLogs(profile_id: string, pageSize: number = 20) {
  ...
  const fetchFeed = useCallback(async () => {
    ...
    try {
      ...
      const likesQuery = supabase.from('log_likes').select('*').in('log_id', logIds);

      const commentsQuery = supabase.from('log_comments').select('*').in('log_id', logIds);

      if (blockedUserIds.length > 0) {
        likesQuery.not('profile_id', 'in', `(${blockedUserIds.join(',')})`);
        commentsQuery.not('profile_id', 'in', `(${blockedUserIds.join(',')})`);
      }
  ...
  const deleteLog = async (logIdToDelete: string) => {
    ...
    try {
      ...
      const { error: deleteError } = await supabase.from('logs').delete().eq('id', logIdToDelete);
      ...
      setProfileLogs((prevProfileLogs) =>
        prevProfileLogs.filter((log) => log.id !== logIdToDelete),
      );
...
export const useLog = (id: string | undefined) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let likesQuery = supabase.from('log_likes').select('*').eq('log_id', id);
